...
import { calculateTrapezoidalVolume } from '@/lib/calculations/hydraulic';
...
export function TrapezoidalChannelCalculator() {
  ...
  const handleCalculate = useCallback(() => {
    const baseMinor = fields.find((f) => f.name === 'baseMinor')?.value ?? 0;
    const baseMajor = fields.find((f) => f.name === 'baseMajor')?.value ?? 0;
    const height = fields.find((f) => f.name === 'height')?.value ?? 0;
    const length = fields.find((f) => f.name === 'length')?.value ?? 0;

    if (baseMinor <= 0 || baseMajor <= 0 || height <= 0 || length <= 0) {
      setFields((prev) =>
        prev.map((f) => ({
          ...f,
    ...
    const result = calculateTrapezoidalVolume(baseMinor, baseMajor, height, length);
